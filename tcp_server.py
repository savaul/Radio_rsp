...
import time
import logging
logging.basicConfig(filename='server.log',filemode='w',format='%(asctime)s %(message)s', level=logging.WARNING)
...
#	print data
	if data[:3]=="AN ":#un prefix pentru identificarea expeditorului - aici radioul
		...
		logging.debug('de la radio %s', data)
#		print data
	elif data[:6]=="ignore":#mesajul ping-ului, just checking ca radioul merge
#		print "ignore"
		reply="ignore"#trebuie? un raspuns
		interval = time.time()-start_time
		logging.debug('de la ping, intervalul de cand n-am auzit de radio  %s, data %s', interval, data)
		if interval > 5:
			n+=1
			print interval, n
		if interval>120:#test de cand n-am mai auzit nimic de la radio
			...
			exec_cmd('sudo pkill -9 -f socket_radio.py')
	                time.sleep(30)
			logging.debug("killing socket radio %s", time.strftime("%H:%M:%S"))
		...
		logging.debug('de la browser %s', data)
...
#    	print 'Connected with ' + addr[0] + ':' + str(addr[1])
	...
	logging.debug('o noua conexiune e acceptata')
