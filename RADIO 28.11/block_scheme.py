...
orar=schedule.read()
orar_start=0
#print orar#aici incarca orarul centralei	
...
def switch_event_menu(p):#main event, am apasat butonul de meniu
	...
	tick = time.time()#asemenea
	if MERGE==1:#daca radioul merge
...
#	print "8"#test sa vad ca trece pe aici
...
def centrala():
	global orar, orar_start, AUTO, onoff
	ziua=strftime("%A")
...
#	print interval1
	...
	if AUTO=="AUTO":
		if m==1 and action=="OFF":
			exec_cmd('gpio write 12 0')
                if m==0 and action=="ON":
                        exec_cmd('gpio write 12 1')
	...
	if AUTO=="ON":
		if action=="ON":
			onoff=1
		if onoff==1 and m==1 and action=="OFF":
                        exec_cmd('gpio write 12 0')
			onoff=0
			AUTO="AUTO"
	if AUTO=="OFF":
		if action =="OFF":
			onoff=1
		if onoff==1 and m==0 and action=="ON":
                        exec_cmd('gpio write 12 1')	
			onoff=0
			AUTO="AUTO"
#	print action				
#	exista=orar_today.find(ora)
#	if exista !=-1:
#		p=os.popen('gpio read 14')
#		m= int(p.readline())
#		end_action = orar_today.find("\n", exista)
#		action = orar_today[exista+6:end_action]
#		print action	
#		if action=="OFF" and m==1:
#			exec_cmd('gpio write 12 0')
#		if action =="ON" and m==0:
...
#	FLAG=0
	try:
		...
		print "inca o eroare de mail", sys.exc_info()[0], sys.exc_info()[2]
		numMessages=0
        if numMessages != 0:
		nr_new_msg +=1
		msg_blink=time.time()
...
#	exec_cmd('gpio write 14 1')
	...
	if nr_iteratii!=0:
		try:
			...
			rss_line.decode('ascii','ignore')
#			print rss_line
			rss_line=rss_line.replace("\\t", "t")
			rss_line=rss_line.replace("\\sf", "s")
			rss_line=rss_line.replace("\\xe2", "a")
			rss_line=rss_line.replace("\\u0102", "A")
			rss_line=rss_line.replace("\\a", "a")
			rss_line=rss_line.replace("\\se", "S")
			rss_line=rss_line.replace("\\u0219", "s")
			rss_line=rss_line.replace("\\u021b", "t")
	#		rss_line=rss_line.replace("\\", "")
			rss_line=rss_line.replace("\\u0162", "T")
			rss_line=rss_line.replace("\\xc3xae", "i")
			rss_line=rss_line.replace("\\xce", "I")
			rss_line=rss_line.replace("\\xc2", "A")
			rss_line=rss_line.replace("\\u0218", "S")
			rss_line=rss_line.replace("\\u021a", "T")
			rss_line=rss_line.replace(chr(238), "i")
			rss_line=rss_line.replace(chr(140), "i")

#			split= int(len(rss_line)/16)+1
#			for i in range (split):
#				FLAG_RSS = FLAG_RSS+rss_line[16*i:(i+1)*16]+"\n0.7&"
#			FLAG= FLAG_RSS
			space_st=0
			space_fin=0
			while space_fin != -1:
				if len(rss_line[space_st:])>16:
					temp_str=rss_line[space_st:space_st+16]
#					print temp_str
...
#					print last_space
					space_fin = space_st+last_space
					FLAG_RSS=FLAG_RSS+rss_line[space_st:space_fin]+"\n0.5&"
					space_st=space_fin+1
				else:
					FLAG_RSS=FLAG_RSS+rss_line[space_st:]+"\n0.5&"
					break
#		print FLAG_RSS
