...
# $Id: radio4.py,v 1.41 2013/07/12 09:10:45 bob Exp $
#
# Author : Bob Rathbone
...
def display_current(lcd,radio):
	current = radio.getCurrentName()
	current_id = radio.getCurrentID()
	source = radio.getSource()
	errorStr = radio.getErrorString()

	lcd.line4("Volume " + str(radio.getStoredVolume())) 

	if source == radio.RADIO:
		if current_id < 0:
			lcd.line2("Bad input stream!")
		else:
	...
	if current_id < 0:
		...
		if leng > 20:
			lcd.scroll3(current[0:80],interrupt)
		elif  leng < 1:
			lcd.line3("No input source!")
			time.sleep(1)
		else:
			lcd.line3(current)
...
def display_search(lcd,radio):
	...
	if source == radio.PLAYER:
		current_artist = radio.getArtistName(index)
		lcd.scroll2(current_artist[0:80],interrupt) 
		lcd.scroll3(radio.getTrackName(index),interrupt) 
	else:
